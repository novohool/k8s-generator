  ...
  constructor(scope: Construct, id: string, props: ChartProps = {}) {
    ...
    if (secondaryResourceOptions[resourceType] && secondaryResourceOptions[resourceType].length > 0 && secondaryResourceType && secondaryResourceType !== '' && secondaryResourceType !== 'None') {
      switch (secondaryResourceType) {
        ...
        case 'configmap':
          // 补全 ConfigMap 代码
          const configData = parseEnvVars(config.env).reduce((acc, env) => { acc[env.name] = env.value; return acc; }, {} as Record<string, string>);
        ...
        case 'secret':
          // 补全 Secret 代码
          const secretData = config.secretData.split(',').reduce((acc: Record<string, string>, pair) => { const [k, v] = pair.split('='); if (k && v) acc[k.trim()] = v.trim(); return acc; }, {} as Record<string, string>);
